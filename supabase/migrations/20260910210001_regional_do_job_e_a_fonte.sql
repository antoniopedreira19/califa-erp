-- =====================================================================
-- A regional do JOB é a fonte; o rateio só existe onde não há job
--
-- Decisão do Tiago em 10/09/2026, fechando a pendência que a reversão
-- `20260909180001_regional_id_volta_a_ser_nullable.sql` deixou aberta:
-- garantir regional em todo lançamento para o DRE.
--
...
--
-- A tentativa anterior (`regional_id NOT NULL` nas três tabelas) falhou
-- porque mirou o lugar errado: a coluna singular é o último fallback, e
-- torná-la obrigatória quebrou seis fluxos de escrita em 24h. A trava
-- certa é UMA só, e está aqui: se a regional do job é a fonte de tudo
-- que nasce de job, então o JOB é que não pode existir sem regional.
--
-- ---------------------------------------------------------------------
-- 1. `jobs.regional_id` passa a ser NOT NULL
--
-- É a pedra angular da regra. Os 7 jobs do banco já têm regional, o
-- formulário de abertura já exige (`lib/validations/abertura-job.ts`), e
-- a avulsa/recorrente com job já RECUSAM job sem regional.
--
-- ⚠️ O caminho que quebraria, corrigido no mesmo commit: a EDIÇÃO do job
-- (`lib/validations/jobs.ts` + `job-editor-drawer.tsx`) oferecia a opção
-- "Sem regional" e gravava null. Sem esse conserto, esta migration
-- repetiria o episódio de 08/09 — erro cru do banco numa tela que antes
-- salvava. Varridos TODOS os caminhos de escrita em `jobs` antes de
-- aplicar: 1 insert (abertura, já exige) e 11 updates (só a edição
-- tocava em `regional_id`).
--
-- ---------------------------------------------------------------------
-- 2. Desembolso não se vincula mais a job
--
-- "Tudo do job deverá ser lançado pelo job" — o desembolso é o
-- instrumento da despesa SEM job. A coluna `job_id` fica (remover coluna
-- é destrutivo e não foi autorizado), mas passa a ser barrada por CHECK.
-- A tabela está vazia hoje, então não há dado a preservar.
--
-- Consequência que ANDA JUNTO, senão a mudança perde dinheiro de vista:
-- a `vw_fluxo_caixa` lia a regional do desembolso pelo JOB e ignorava
-- `desembolsos_regionais` — o rateio era gravado pelo formulário e nunca
-- lido. Sem job, todo desembolso ficaria sem regional. Agora a view lê o
-- rateio, com fator, como já fazia com a avulsa.
--
-- ---------------------------------------------------------------------
-- 3. A baixa de título passa a herdar a regional do job
--
-- `dar_baixa_titulo` insere em `lancamentos_financeiros` sem `job_id` e
-- sem `regional_id`. A view já derivava o job certo pelo título ->
-- `faturamento_itens` (CTE `lancamento_job`, que inclusive rateia entre
-- N jobs quando a nota cobre vários) — mas jogava fora essa informação
-- na hora da regional, que saía do CTE `lancamento_rateio`, cego para a
-- derivação. Resultado: TODA receita realizada entrava no DRE sem
-- regional. Agora os dois CTEs se encontram:
--
--     COALESCE(jlj.regional_id, lr.regional_id)
--
-- ...com `jlj` pendurado em `lancamento_job.job_id`, que é o job direto
-- OU o derivado. Como `jobs.regional_id` virou NOT NULL, esse COALESCE
-- nunca cai para o segundo termo quando existe job.
--
-- ---------------------------------------------------------------------
-- O que NÃO muda de número
--
-- A ordem dos CTEs de rateio foi invertida (job primeiro, rateio depois)
-- para o código dizer o mesmo que a regra. Isso NÃO altera resultado
-- nenhum hoje: a escrita da avulsa e da recorrente já força rateio único
-- de 100% na regional do job quando há job. A inversão é para a próxima
-- pessoa que ler, e para o caso de a escrita um dia divergir.
--
-- ---------------------------------------------------------------------
-- Fora do escopo, de propósito (próximo lote do Tiago)
--
-- - COMO o rateio é preenchido em desembolso, recorrência e avulsa sem
--   job (a tela e a regra de preenchimento).
-- - Despesa sem job e SEM rateio: hoje sai com regional nula, e a view
--   preserva a linha em vez de sumir com o dinheiro. Não há trava de
--   "pelo menos uma linha de rateio", nem trigger de soma 100 em
--   `desembolsos_regionais` (avulsa e recorrente têm).
-- - Pagamento de fatura de cartão (agrega N compras) e título com origem
--   `bv`/`avulso`.
-- =====================================================================

-- ---------------------------------------------------------------------
-- 1. A regional do job passa a ser obrigatória
-- ---------------------------------------------------------------------
alter table public.jobs alter column regional_id set not null;

comment on column public.jobs.regional_id is
  'FONTE da regional de tudo que nasce deste job — PP, avulsa, título, previsão, save. NOT NULL desde 10/09/2026: a regra é "origem em job -> regional do job, 100%", e ela não se sustenta se o job puder não ter regional. Ver 20260910210001.';

-- ---------------------------------------------------------------------
-- 2. Desembolso é o instrumento da despesa SEM job
-- ---------------------------------------------------------------------
alter table public.desembolsos
  add constraint desembolso_nao_tem_job check (job_id is null);

comment on column public.desembolsos.job_id is
  'SEM USO desde 10/09/2026 — barrada pelo CHECK `desembolso_nao_tem_job`. Tudo que é do job se lança pelo job (PP); o desembolso é a despesa que não tem job. A coluna ficou no lugar porque removê-la é destrutivo; a regional do desembolso vem de `desembolsos_regionais`.';

comment on table public.desembolsos_regionais is
  'O rateio de regional do desembolso. Passou a ser LIDO pela vw_fluxo_caixa em 10/09/2026 — antes era gravado pelo formulário e a view usava a regional do job, que o desembolso nem tem mais.';
