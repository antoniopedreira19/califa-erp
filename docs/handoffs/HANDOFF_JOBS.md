...
| **Lista de Jobs: descritivos** | ⚠️ **Novo em 04/09/2026** (decisão 051). Um ícone ao lado do nome abre o descritivo num **cartão ancorado** sobre a tabela: o do **projeto na faixa do grupo** (`projetos.descricao`) e o do **job na linha** (`jobs.observacoes`). Um por vez; fecha com clique fora, X ou `Esc`, e o clique dentro NÃO fecha (dá para copiar o texto). Do cartão do job há um salto para o do projeto. Registro anterior à obrigatoriedade da 043 fica com o **ícone apagado** — hoje 27 dos 30 jobs. A busca **não** olha o descritivo (decisão do Tiago, 04/09). Componente: `components/ui/descritivo-popover.tsx`. |
| **Lista de Jobs: faixa do projeto** | ⚠️ **Corrigido em 04/09/2026.** A faixa usava `colSpan={10}` numa tabela de **12 colunas** e morria antes de "Valor total" e "Status", deixando o fim de cada linha de projeto em branco. |
