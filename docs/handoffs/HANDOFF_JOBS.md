...
| **Decomposição do REALIZADO** | ⚠️ **Mudou em 01/09/2026** (decisão 035 §7). As colunas R$ Unit., QT e D/M do REALIZADO **somam** o trio das PPs não canceladas do item, em vez de derivar por rateio (`dias_meses` fixo em 1 e unitário = total ÷ QT). Com uma PP no item, a linha fica idêntica ao que o GP digitou. **Com várias, a linha deixa de multiplicar:** `total_realizado` é a soma dos `valor`, não o produto das somas — item com 2 PPs somando R$ 18.000 exibe `R$ 48.000,00 × 0,75 × 2`. Aceito porque nenhum cálculo do sistema tira dinheiro dessas três colunas. |
