...
function PreviewPanel({
  ...
  return (
    <div className="space-y-5">
      ...
      {/* DEBUG TEMPORÁRIO: dump do cell.value das linhas com decimal suspeito */}
      {preview.debug_samples && preview.debug_samples.length > 0 && (
        <div className="rounded-xl border border-blue-200 bg-blue-50 overflow-hidden">
          <div className="px-4 py-2 border-b border-blue-200 text-xs font-semibold text-blue-800">
            🔬 Debug — amostras cruas do cell.value (col D)
          </div>
          <div className="max-h-96 overflow-y-auto">
            {preview.debug_samples.map((s, i) => (
              <div
                key={i}
                className="border-b border-blue-100 px-4 py-2 text-[11px] font-mono text-blue-900 space-y-0.5"
              >
                <div>
                  <b>L{s.linha}</b> · {s.item}
                </div>
                <div>
                  typeof: <b>{s.colD_typeof}</b> · parsed:{" "}
                  <b>{s.colD_parsed}</b>
                </div>
                <div className="break-all">raw: {s.colD_raw}</div>
              </div>
            ))}
          </div>
        </div>
