 ...
 *
 * As linhas entram "como o financeiro vê" (decisão 099): pedido de save
 * `job_aberto` que aguarda é desfeito na conta, menos os de `contar` — a
 * aprovação conta o pedido que está aprovando. É a mesma regra do
 * faturamento previsto do formulário; sem ela, com um consumo aguardando,
 * o imposto sairia sobre uma base e o faturamento sobre outra.
 */
...
export async function impostoDoJob(
  ...
  const [jobRes, itensRes, pedidos] = await Promise.all([
    supabase
      .from("jobs")
      .select(
    ...
    pedidosParaFinanceiroDoJob(supabase, tenantId, jobId).catch((e: Error) => {
      console.error("[abertura-job.imposto.pedidos]", e.message);
      return null;
    }),
  ]);
  ...
  // Sem os pedidos o imposto sairia sobre a base da produção em silêncio.
  ...
  const vistos = itensParaOFinanceiro(
